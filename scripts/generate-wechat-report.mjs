...
async function main() {
    ...
    // 保存到 www/ 目录（被 Git 跟踪，clawbot 可直接拉取）
    const reportDir = path.join(ROOT, 'www');
    await fs.mkdir(reportDir, { recursive: true });
    const reportPath = path.join(reportDir, 'wechat_report.md');
    await fs.writeFile(reportPath, report, 'utf8');
    
    console.log(`✅ WeChat report generated: ${reportPath}`);
